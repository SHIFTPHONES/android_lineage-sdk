...
/**
 * The ShiftOsSettingsProvider serves as a {@link ContentProvider} for ShiftOs specific settings
 */
public class ShiftOsSettingsProvider extends ContentProvider {
    public static final String TAG = "ShiftOsSettingsProvider";
    ...
    protected final SparseArray<ShiftOsDatabaseHelper> mDbHelpers = new SparseArray<>();
    ...
    static {
        sUriMatcher.addURI(ShiftOsSettings.AUTHORITY, ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_SYSTEM,
                SYSTEM);
        sUriMatcher.addURI(ShiftOsSettings.AUTHORITY, ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_SECURE,
                SECURE);
        sUriMatcher.addURI(ShiftOsSettings.AUTHORITY, ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_GLOBAL,
                GLOBAL);
        sUriMatcher.addURI(ShiftOsSettings.AUTHORITY, ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_SYSTEM +
    ...
    public boolean onCreate() {
        if (LOCAL_LOGV) Log.d(TAG, "Creating ShiftOsSettingsProvider");
        ...
        mUriBuilder.authority(ShiftOsSettings.AUTHORITY);
    ...
    public Bundle call(String method, String request, Bundle args) {
        ...
        if (args != null) {
            int reqUser = args.getInt(ShiftOsSettings.CALL_METHOD_USER_KEY, callingUserId);
        ...
        switch (method) {
            // Get methods
            case ShiftOsSettings.CALL_METHOD_GET_SYSTEM:
                return lookupSingleValue(callingUserId, ShiftOsSettings.System.CONTENT_URI,
                        request);
            case ShiftOsSettings.CALL_METHOD_GET_SECURE:
                return lookupSingleValue(callingUserId, ShiftOsSettings.Secure.CONTENT_URI,
                        request);
            case ShiftOsSettings.CALL_METHOD_GET_GLOBAL:
                return lookupSingleValue(callingUserId, ShiftOsSettings.Global.CONTENT_URI,
                        request);

            // Put methods
            case ShiftOsSettings.CALL_METHOD_PUT_SYSTEM:
                enforceWritePermission(shiftos.platform.Manifest.permission.WRITE_SETTINGS);
                callHelperPut(callingUserId, ShiftOsSettings.System.CONTENT_URI, request, args);
                return null;
            case ShiftOsSettings.CALL_METHOD_PUT_SECURE:
                enforceWritePermission(
                        shiftos.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                callHelperPut(callingUserId, ShiftOsSettings.Secure.CONTENT_URI, request, args);
                return null;
            case ShiftOsSettings.CALL_METHOD_PUT_GLOBAL:
                enforceWritePermission(
                        shiftos.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                callHelperPut(callingUserId, ShiftOsSettings.Global.CONTENT_URI, request, args);
                return null;

            // List methods
            case ShiftOsSettings.CALL_METHOD_LIST_SYSTEM:
                return callHelperList(callingUserId, ShiftOsSettings.System.CONTENT_URI);
            case ShiftOsSettings.CALL_METHOD_LIST_SECURE:
                return callHelperList(callingUserId, ShiftOsSettings.Secure.CONTENT_URI);
            case ShiftOsSettings.CALL_METHOD_LIST_GLOBAL:
                return callHelperList(callingUserId, ShiftOsSettings.Global.CONTENT_URI);

            // Delete methods
            case ShiftOsSettings.CALL_METHOD_DELETE_SYSTEM:
                enforceWritePermission(shiftos.platform.Manifest.permission.WRITE_SETTINGS);
                return callHelperDelete(callingUserId, ShiftOsSettings.System.CONTENT_URI,
                        request);
            case ShiftOsSettings.CALL_METHOD_DELETE_SECURE:
                enforceWritePermission(
                        shiftos.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                return callHelperDelete(callingUserId, ShiftOsSettings.Secure.CONTENT_URI,
                        request);
            case ShiftOsSettings.CALL_METHOD_DELETE_GLOBAL:
                enforceWritePermission(
                        shiftos.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                return callHelperDelete(callingUserId, ShiftOsSettings.Global.CONTENT_URI,
                        request);
        }
    ...
    private Cursor queryForUser(int userId, Uri uri, String[] projection, String selection,
        ...
        ShiftOsDatabaseHelper dbHelper = getOrEstablishDatabase(getUserIdForTable(tableName, userId));
    ...
    private Uri insertForUser(int userId, Uri uri, ContentValues values) {
        ...
        if (ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_GLOBAL.equals(tableName)) {
            validateGlobalSettingNameValue(name, value);
        } else if (ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_SYSTEM.equals(tableName)) {
            validateSystemSettingNameValue(name, value);
        } else if (ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_SECURE.equals(tableName)) {
    ...
    private int deleteForUser(int callingUserId, Uri uri, String selection,
        ...
        if (!TextUtils.isEmpty(selection) && selectionArgs.length > 0) {
            ...
            ShiftOsDatabaseHelper dbHelper = getOrEstablishDatabase(getUserIdForTable(tableName,
                    callingUserId));
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        // NOTE: update() is never called by the front-end ShiftOsSettings API, and updates that
    ...
    /**
     * Tries to get a {@link ShiftOsDatabaseHelper} for the specified user and if it does not exist, a
     * new instance of {@link ShiftOsDatabaseHelper} is created for the specified user and returned.
     * @param callingUser
     * @return
     */
    private ShiftOsDatabaseHelper getOrEstablishDatabase(int callingUser) {
        ...
        try {
            ShiftOsDatabaseHelper dbHelper;
            synchronized (this) {
    ...
    /**
     * Check if a {@link ShiftOsDatabaseHelper} exists for a user and if it doesn't, a new helper is
    ...
    private void establishDbTracking(int userId) {
        ...
        synchronized (this) {
            ...
            if (LOCAL_LOGV) {
                Log.i(TAG, "Checking ShiftOS settings db helper for user " + userId);
            }
            if (dbHelper == null) {
                if (LOCAL_LOGV) {
                    Log.i(TAG, "Installing new ShiftOS settings db helper for user " + userId);
                }
                dbHelper = new ShiftOsDatabaseHelper(getContext(), userId);
    ...
    private void checkWritePermissions(String tableName) {
        ...
        final boolean protectedTable =
                ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_SECURE.equals(tableName) ||
                ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_GLOBAL.equals(tableName);
        ...
        throw new SecurityException(
                String.format("Permission denial: writing to ShiftOS settings requires %1$s",
                        shiftos.platform.Manifest.permission.WRITE_SECURE_SETTINGS));
    }
    ...
    private String getTableNameFromUriMatchCode(int code) {
        switch (code) {
            case SYSTEM:
            case SYSTEM_ITEM_NAME:
                return ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_SYSTEM;
            case SECURE:
            case SECURE_ITEM_NAME:
                return ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_SECURE;
            case GLOBAL:
            case GLOBAL_ITEM_NAME:
                return ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_GLOBAL;
            default:
                throw new IllegalArgumentException("Invalid uri match code: " + code);
        }
    }
    ...
    private int getUserIdForTable(String tableName, int userId) {
        return ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_GLOBAL.equals(tableName) ?
                UserHandle.USER_SYSTEM : userId;
    }

    /**
     * Modify setting version for an updated table before notifying of change. The
     * {@link ShiftOsSettings} class uses these to provide client-side caches.
     * @param uri to send notifications for
     * @param userId
     */
    private void notifyChange(Uri uri, String tableName, int userId) {
        String property = null;
        final boolean isGlobal = tableName.equals(ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_GLOBAL);
        if (tableName.equals(ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_SYSTEM)) {
            property = ShiftOsSettings.System.SYS_PROP_SHIFTOS_SETTING_VERSION;
        } else if (tableName.equals(ShiftOsDatabaseHelper.ShiftOsTableNames.TABLE_SECURE)) {
            property = ShiftOsSettings.Secure.SYS_PROP_SHIFTOS_SETTING_VERSION;
        } else if (isGlobal) {
            property = ShiftOsSettings.Global.SYS_PROP_SHIFTOS_SETTING_VERSION;
        }
    ...
    private void validateGlobalSettingNameValue(String name, String value) {
        ShiftOsSettings.Validator validator = ShiftOsSettings.Global.VALIDATORS.get(name);
    ...
    private void validateSystemSettingNameValue(String name, String value) {
        ShiftOsSettings.Validator validator = ShiftOsSettings.System.VALIDATORS.get(name);
        if (validator == null) {
    ...
    private void validateSecureSettingValue(String name, String value) {
        ShiftOsSettings.Validator validator = ShiftOsSettings.Secure.VALIDATORS.get(name);
